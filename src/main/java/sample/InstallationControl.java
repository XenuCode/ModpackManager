...
//
    ...
    public ObservableList<String> getObservableList(ObservableList<String> list)
    {
        float averageMatch= SearchControl.getAverageMatch(modpacks);
        System.out.println(averageMatch);
        list.clear();
        for (int i = 0; i <modpacks.size(); i++) {
            if(i>=10 || modpacks.get(i).getPhraseMach()<= 20)
            {
                break;
            }
